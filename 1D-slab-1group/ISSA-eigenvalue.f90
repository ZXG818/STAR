...
!****************************************************************************
    ...
    type :: meshtype
        ...
        real(8) :: sigma_f         ! fission marco-xs
    ...
    subroutine MacroXS(mesh_group, x_lower, x_upper, &
                       SIGMA_T, SIGMA_S, SIGMA_F, external_source)
    implicit none
    type(meshtype),dimension(:,:),intent(inout) :: mesh_group
            ...
            if((mod(i,2).eq.0) .and. (mod(j,2).eq.0)) then
                if(mesh_group(i,j)%x .LT. x_upper) then
                    if(mesh_group(i,j)%x .GT. x_lower) then
                        ...
                        mesh_group(i,j)%sigma_f = SIGMA_F
            ...
            if((mod(i,2).eq.0) .and. (mod(j,2).eq.0)) then
                do k=1, angular_cnt
                    if(mod(k,2).eq.0) then
                        scalar_flux = scalar_flux + mesh_group(k, j)%flux * omega(k/2)
                    endif
                enddo
                ! scattering source term
                mesh_group(i,j)%scatter_source = mesh_group(i,j)%sigma_s * scalar_flux
                ! fission source term
                mesh_group(i,j)%fission_source = mesh_group(i,j)%sigma_f * scalar_flux
                ...
                mesh_group(i,j)%total_source = mesh_group(i,j)%scatter_source  &
                                             ...
                                             + mesh_group(i,j)%fission_source / keff
            endif
        enddo
    enddo
    return 
    ...
    real(8) :: sum_fission_source = 0.d0
    real(8) :: old_sum_fission_source = 0.d0
    integer :: i, j, k
    integer :: angular_cnt, x_cnt
    angular_cnt = size(mesh_group,1)
    x_cnt = size(mesh_group,2)
    ...
    do i=1, angular_cnt
        do j=1, x_cnt
            if((mod(i,2).eq.0) .and. (mod(j,2).eq.0)) then
                sum_fission_source = sum_fission_source + mesh_group(i,j)%fission_source
                old_sum_fission_source = old_sum_fission_source+old_mesh_group(i,j)%fission_source
            endif
        enddo
    enddo
    ...
    subroutine PowerIteration(mesh_group, old_mesh_group, length, REGION, &
                              angular_flux_guess, keff_guess)
    implicit none
    type(meshtype), dimension(:, :), intent(inout) :: mesh_group
    type(meshtype), dimension(:, :), intent(inout) :: old_mesh_group
    real(8), intent(in) :: length
    type(regiontype), dimension(:), intent(in) :: REGION
    real(8), intent(in) :: angular_flux_guess
    ...
    do i=1, region_cnt
        call MacroXS(mesh_group, region(i)%x_lower, region(i)%x_upper, &
                    region(i)%sigma_t, region(i)%sigma_s,              &
                    region(i)%sigma_f, region(i)%external_source)
    enddo
    ! initialize the field
    call Initialize(mesh_group, angular_flux_guess)
    ...
    do i=1, MAX_ITER
        ...
        call SolveRightSide(mesh_group, SPECIFIED_BOUND, 0.d0)
        ...
        write(*, "(A15,I3,A15,G15.7,A20,G15.7)") "Iteration=", i,       &
                                                 "Residual=", residual, &
                                                 "K-eigenvalue=", keff
        old_mesh_group = mesh_group
        old_keff = keff
    ...
    region(1)%x_upper = 2.0d0
    region(1)%sigma_t = 1.d0
    region(1)%sigma_s = 0.5d0
    region(1)%sigma_f = 1.0d0
    region(1)%external_source = 0.d0
    
    region(2)%x_lower = 2.d0
    region(2)%x_upper = 5.d0
    region(2)%sigma_t = 0.8d0
    region(2)%sigma_s = 0.4d0
    region(2)%sigma_f = 0.d0
    region(2)%external_source = 0.d0
    
    ! PI method
    call PowerIteration(mesh_group, old_mesh_group, length, region, 1.d0, 1.d0)
